...
class TestBootStrap(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        path = os.path.join(
            get_root_directory(),
            'tests/fit/test_result.obj'
        )

        with open(path, 'rb') as file:
            cls.result = pickle.load(file)

        cls.boot = BootStrap(cls.result)
    ...
    def test__details(self):
        ...
        self.assertEqual(self.result.details['deconv'], deconv)
        self.assertEqual(self.result.details['maxfev'], maxfev)
    ...
    def test__get_division_number(self, val):
        if val in [10, 15, 20, 25, 33, 50]:
            ...
            with self.assertRaises(ExperimentException) as context:
                self.boot._get_division_number(val)
    ...
    def test__initial_final_values(self, names, only_vary):
        ...
        ini, final = self.boot._initial_final_values(
            self.result.estimation_params, names, only_vary)
    ...
    def test_generate_data_sets(self, n_boots, size, data_from):
        boot_2 = BootStrap(self.result)
        data2 = boot_2.generate_datasets(n_boots, size, data_from, True)
    ...
    def test_fit_bootstrap(self):
        ...
        self.assertEqual(self.data.shape[2], 3)
