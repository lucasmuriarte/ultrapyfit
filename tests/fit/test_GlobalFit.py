...
class TestGlobalFit(ArrayTestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        for i in range(10):
            cls.data_simulated[:, i] = \
                model.expNDataset(cls.params, i)

    def test__generate_residues(self):
        fitter = GlobalFit(
            self.time_simulated, self.data_simulated, 3, self.params)
...
class TestGlobalFitExponential(TestGlobalFit):
    def test_global_fit(self):
        ...
        fitter = GlobalFitExponential(
            self.time, self.data_select, 3, parameters, False,
    ...
    def test__objective(self):
        fitter = GlobalFitExponential(
            self.time_simulated, self.data_simulated, 3,
            self.params, False)
    ...
    def test__pre_fit(self):
        params = GlobExpParameters(
            self.data_select.shape[1], [4, 40, 400])
