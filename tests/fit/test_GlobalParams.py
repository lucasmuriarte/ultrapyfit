...
from ultrafast.fit.GlobalParams import \
    GlobExpParameters, \
    GlobalTargetParameters
...
class TestGlobExpParameters(unittest.TestCase):
    ...
    @parameterized.expand([[0.12, False, 1E12],
                           [0.18, True, None],
                           [0.15, False, 1E8]])
    def test__add_deconvolution(self, fwhm, opt_fwhm, tau_inf):
        params = GlobExpParameters(self.n_traces, self.taus)
    ...
    @parameterized.expand([
        [0, True, 0.12, False, True, 1E12, None],
        [0, False, None, True, True, None, 0],
        [0, True, None, True, False, None, 5],
        [0.5, True, 0.18, False, False, 1E12, 10]])
    def test_adjustParams(
            self, t0, vary_t0, fwhm, opt_fwhm, GVD_corrected, tau_inf, y0):

        params = GlobExpParameters(self.n_traces, self.taus)
        params.adjustParams(
            t0, vary_t0, fwhm, opt_fwhm, GVD_corrected, tau_inf)
...
class TestGlobalTargetParams(TestGlobExpParameters):
    ...
    def test__add_preexp_t0_y0(self, t0, vary, gvd_corrected):
        ...
        # two parameter per tau including t0 thus: len(taus) + 1
        number = len(self.params_model) + 2 * self.n_traces \
            + self.exp_no * (self.n_traces + 1)
    ...
    @parameterized.expand([[0.12, False],
                           [0.18, True],
                           [0.15, False]])
    def test__add_deconvolution(self, fwhm, opt_fwhm):
        params = GlobalTargetParameters(self.n_traces, self.model)
    ...
    def test_adjustParams(
            self, t0, vary_t0, fwhm, opt_fwhm, GVD_corrected):

        params = GlobalTargetParameters(self.n_traces, self.model)
        params.adjustParams(
            t0, vary_t0, fwhm, opt_fwhm, GVD_corrected)
