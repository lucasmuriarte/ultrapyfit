...
from ultrafast.old.ExponentialFit import \
    globalfit_exponential, \
...
class TestGlobalFitClass(ArrayTestCase):
    @classmethod
    def setUpClass(cls):
        path = os.path.join(
            get_root_directory(), 'examples/data/denoised_2.csv')

        path2 = os.path.join(
            get_root_directory(), 'examples/data/gauss_denoised.csv')

        cls.original_taus = [8, 30, 200]

        cls.time, cls.data, cls.wave = read_data(
            path, wave_is_row=True)

        cls.data_select, cls.wave_select = select_traces(
            cls.data, cls.wave, 'auto')

        cls.time_gauss, cls.data_gauss, cls.wave_gauss = read_data(
            path2, wave_is_row=True)

        cls.data_select_gauss, cls.wave_select_gauss = select_traces(
            cls.data_gauss, cls.wave_gauss, 10)
    ...
    def test_globalfit_gauss_exponential(self):
        ...
        self.assertNearlyEqualArray(self.original_taus, final_taus, 3)
