...
class TestBasicSpectrum(unittest.TestCase):
    ...
    def test__addDeconvolution(self, fwhm, opt_fwhm, tau_inf):
        ...
        # notice tau_inf is not a parameter and thus is not added only its
        # preexponential function plus fwhm
        number = (len(taus) + 2) * 2 * number_traces
        if tau_inf is None:
            number -= number_traces
        self.assertEqual(len(params.params), number)
    ...
    def test_adjustParams(
        ...
        if fwhm is not None:
            ...
            number = number + number_traces * \
                2 if tau_inf is not None else number + number_traces
            self.assertEqual(params.params['fwhm_1'].vary, opt_fwhm)
            self.assertEqual(params.params['fwhm_1'].value, fwhm)
            self.assertEqual(params.params['fwhm_1'].value, fwhm)
            if GVD_corrected:
