...
from ultrafast.tools import readData
...
class TestPlotTransientClass(unittest.TestCase):
    ...
    def test_plot_spectra_list_auto(
            self,
            times,
            rango,
            from_max_to_min,
    ...
    def test__time_to_real_times(
            self,
            times,
            rango,
            include_max,
            from_max_to_min):
        result = explorer._time_to_real_times(
            times, rango, include_max, from_max_to_min)
        if isinstance(times, list):
            if times[0] == 'auto':
                wave = None if len(times) == 2 else times[2]
                output = explorer._get_auto_points(
