import unittest
import numpy as np
from ultrafast.PreprocessingClass import Preprocessing, ExperimentException
...
class TestPreprocessing(unittest.TestCase):
    ...
    def test_baselineSubstraction(self, number_spec, only_one):
        ...
        result = Preprocessing.baseline_substraction(
            data_wave, number_spec, only_one)
        exp = data_wave * 0.0
        if only_one or number_spec == 0:
            resta = np.ones((1, 150)) * number_spec
        else:
            if isinstance(number_spec, int):
    ...
    def test_delPoints(self, points):
        ...
        if points[0] < 349:
            data_res, vec_res = Preprocessing.del_points(
                points, data_time, time)
            vector = time * 1.0
            axis = 0
        else:
            data_res, vec_res = Preprocessing.del_points(
                points, data_wave, wave)
            axis = 1
            vector = wave * 1.0
