# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 11:59:48 2020
...
y=gauss(x,1,500,50)+gauss(x,1.2,550,20)+gauss(x,0.6,630,30)-0.2+np.random.normal(size=len(x), scale=0.01) 
y2=gauss(x,1.1,400,40)+gauss(x,1,550,60)+gauss(x,0.6,580,40)+0.2+np.random.normal(size=len(x), scale=0.01) 
...
class TestBasicSpectrum(unittest.TestCase):
    ...
    def test_minus(self):
        resta=spec-spec2
        self.assertTrue(self.assertEqualArray(resta.data_table['y'].values, y-y2))
    
    def test_plus(self):
        suma=spec+spec2
        self.assertTrue(self.assertEqualArray(suma.data_table['y'].values, y+y2))
    
    def test_mul(self):
        mul=spec*3
        self.assertTrue(self.assertEqualArray(mul.data_table['y'].values, y*3))
    
    def test_div(self):
        div=spec/5
        self.assertTrue(self.assertEqualArray(div.data_table['y'].values, y/5))
    ...
    def test_obtainMax_high(self):
        self.assertEqual(round(spec.obtainMax(high=600)[1],3),round(y[:350].max(),3))
   
    def test_obtainMax_low(self):
        self.assertEqual(round(spec.obtainMax(low=600)[1],3),round(y[350:].max(),3))  
        
    def test_obtainMin(self):
        self.assertEqual(round(spec.obtainMin()[1],3),round(y.min(),3))
    
    def test_obtainMin_high(self):
        self.assertEqual(round(spec.obtainMin(high=600)[1],3),round(y[:350].min(),3))
   
    def test_obtainMin_low(self):
        self.assertEqual(round(spec.obtainMin(low=600)[1],3),round(y[350:].min(),3))
    ...
    def test_calculateArea(self):
        self.assertTrue(self.assertEqualArray(spec.calculateArea(),np.trapz(y,x)))
        
    def test_calculateArea_low_high(self):
        self.assertTrue(spec.calculateArea(500,700),np.trapz(y[250:451],x[250:451]))
        
    def test_averageOfRange_low_high(self):
        self.assertEqual(round(spec.averageOfRange(500,700),5),round(np.mean(y[250:451]),5))
