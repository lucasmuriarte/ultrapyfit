...
from ultrafast.tools import define_weights, UnvariableContainer, LabBook, book_annotate
...
class Experiment(ExploreData, ExploreResults):
    ...
    def _initialized(self):
        ...
        self.average_time = book_annotate(
            self.preprocessing_report)(
            self.average_time)
        self.derivate_data = book_annotate(
            self.preprocessing_report)(
            self.derivate_data)
        self.cut_wavelength = book_annotate(
            self.preprocessing_report)(
            self.cut_wavelength)
        self.del_points = book_annotate(
            self.preprocessing_report)(
            self.del_points)
        self.shitTime = book_annotate(
            self.preprocessing_report)(
            self.shit_time)
