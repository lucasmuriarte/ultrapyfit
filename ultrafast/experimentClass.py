...
class Experiment(ExploreData, ExploreResults):
    ...
    def _initialized(self):
        self.data_sets.original_data = UnvariableContainer(
        ...
        self.baseline_substraction = book_annotate(self.preprocessing_report)(
            self.baseline_substraction
        )
        self.cut_time = book_annotate(self.preprocessing_report)(self.cut_time)
        self.average_time = book_annotate(self.preprocessing_report)(
            self.average_time
        )
        self.derivate_data = book_annotate(self.preprocessing_report)(
            self.derivate_data
        )
        self.cut_wavelength = book_annotate(self.preprocessing_report)(
            self.cut_wavelength
        )
        self.del_points = book_annotate(self.preprocessing_report)(
            self.del_points
        )
        self.shitTime = book_annotate(self.preprocessing_report)(
            self.shit_time
        )
    ...
    def type_fit(self):
        if not self._params_initialized:
            ...
            return \
                f"parameters for {self._params_initialized} " \
    ...
    def baseline_substraction(self, number_spec=2, only_one=False):
        ...
        new_data = Preprocessing.baseline_substraction(
            self.data, number_spec=number_spec, only_one=only_one
        )
        self.data = new_data
    ...
    def average_time(self, starting_point, step, method="log", grid_dense=5):
        ...
        new_data, new_x = Preprocessing.average_time_points(
            self.data, self.x, starting_point, step, method, grid_dense
        )
        self.data, self.x = new_data, new_x

    def derivate_data(
        self, window_length=25, polyorder=3, deriv=1, mode="mirror"
    ):
        self._add_to_data_set("before_derivation")
        self._add_action("derivate data")
        new_data = Preprocessing.derivateData(
            window_length, polyorder, deriv, mode
        )
        self.data = new_data
    ...
    def cut_wavelength(self, left=None, right=None, innercut=False):
        ...
        new_data, new_wave = Preprocessing.cut_columns(
            self.data, self.wavelength, left, right, innercut
        )
    ...
    def del_points(self, points, dimension="time"):
        ...
        if dimension == "time":
            new_data, new_x = Preprocessing.del_points(
                points, self.data, self.x
            )
        elif dimension == "wavelength":
            new_data, new_wave = Preprocessing.del_points(
                points, self.data, self.wavelength
            )
        else:
            raise ExperimentException(
                'dimension should be "time" or "wavelength"'
            )
        self.data, self.x = new_data, new_x
    ...
    def _readapt_params(self):
        if self._params_initialized == "Exponential":
            ...
            self.initialize_exp_params(
                t0, fwhm, *previous_taus, tau_inf=tau_inf, opt_fwhm=opt_fwhm
            )
        elif self._params_initialized == "Target":
            print("to be coded")
            # to do self.initialize_target_params()
        else:
            pass

    def initialize_exp_params(
        self, t0, fwhm, *taus, tau_inf=1e12, opt_fwhm=False
    ):
        taus = list(taus)
        self._last_params = {
            "t0": t0,
        ...
        param_creator.adjustParams(
            t0, vary_t0, fwhm, opt_fwhm, self.GVD_corrected, tau_inf
        )
        self.params = param_creator.params
        self._params_initialized = "Exponential"
        self._add_action(
            f"new {self._params_initialized} parameters initialized"
        )

    def initialize_target_params(
        self, t0, fwhm, *taus, tau_inf=1e12, opt_fwhm=False
    ):
        pass
    ...
    def final_fit(self, vary=True, maxfev=5000, apply_weights=False):
        if self._params_initialized == "Exponential":
            minimizer = GlobalFitExponential(
                self.x,
                self.selected_traces,
                self._exp_no,
                self.params,
            ...
            minimizer = GlobalFitTargetModel(
                self.selected_traces, self.selected_wavelength, self.params
            )
        else:
            raise ExperimentException(
                'Parameters need to be initiliazed first"'
            )
        if apply_weights:
