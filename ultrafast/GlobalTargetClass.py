...
class GlobalFitTargetModel(lmfit.Minimizer, ModelCreator):
    def __init__(
        ...
        **kwargs
        ...
        lmfit.Minimizer.__init__(
            self, self.objectiveTagetFit, params, nan_policy="propagate"
        )
    ...
    def objectiveTagetFit(params, shared_t0=True):
        ...
        if self.deconv:
            if shared_t0:
                ...
                expvects = [
                    coeffs[i]
                    * self.expGauss(self.x - t0, -1 / eigs[i], fwhm / 2.35482)
                    for i in range(len(eigs))
                ]
                concentrations = [
                    sum(
                        [
                            eigenmatrix[i, j] * expvects[j]
    ...
    def preFit(self):
        ...
        kmatrix = np.array(
            [
                [
                    fit_params["k_%i%i" % (i + 1, j + 1)].value
