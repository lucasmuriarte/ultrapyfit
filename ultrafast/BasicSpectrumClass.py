...
class BasicSpectrum:
    ...
    x: array type independent variable of the spectrum

    y: array type independent variable of the spectrum

    data_table: pandas dataFrame data frame containig x and y with coulmn names
        x:'wavelengths' y:'absorbances'

    sort: bool if True sperctrum is order from the x minimum value to the x
        highest

    name: str name given to the spectrum

    name_x: str name of the x vector, i.e. 'Wavelngth (nm)'

    name_y: str name of the y vector, i.e. 'Absorbance' Methods
    -------
    ...
    def __init__(self, x, y, sort=True, name=None, name_x=None, name_y=None):
        ...
        if sort:
            self.sort = True
            self.data_table = (
                pd.DataFrame({self.name_x: x, self.name_y: y})
                .sort_values(by=[self.name_x])
    ...
    def _minMaxIndex(self, low, high):
        ...
        high_index = (
            self.data_table[self.name_x] - high
        ).abs().sort_values().index[0] + 1
        low_index = (
            self.data_table[self.name_x] - low
        ).abs().sort_values().index[0] - self.data_table.index[0]
    ...
    def transformXvector(self, func, new_name, undo=False):
        """make a tranformation of x verto according to the function pass
         ...
         Paramaters
        ----------
        func: func(x): function that tranform x

        new_name: str string of the new x vector

        undo: bool if True and x has been transform, undo the tranformation, No
            need to pass func or new_name if True
        ...
        elif not self.wavenumber_calculated:
            self._xtranformed = True
            self.old_x_name = copy(self.name_x)
            self.name_x = new_name
            self.data_table[self.name_x] = self.data_table[
                self.old_x_name
    ...
    def cut(self, low=None, high=None, itself=True):
        ...
        Paramaters
        ----------
        low: float minimum value of x

        high: float maximum value of x

        itself: bool if true cut the spectrum else returns a new spectrum
            object

        returns
        -------
        float, float x-position; y-maximum
        """
        ...
        if itself:
            ...
            return BasicSpectrum(
                data_table[self.name_x],
                data_table[self.name_y],
                True,
                self.name,
                self.name_x,
                self.name_y,
    ...
    def __sub__(self, obj):
        ...
        ret_obj = BasicSpectrum(
            self.data_table[self.name_x],
            self.data_table[self.name_y],
            True,
            self.name,
            self.name_x,
            self.name_y,
        )
        ret_obj.data_table[ret_obj.name_y] = (
            self.data_table[self.name_y] - obj.data_table[obj.name_y]
        )
        return ret_obj
    ...
    def __add__(self, obj):
        ...
        ret_obj.data_table[ret_obj.name_y] = (
            ret_obj.data_table[ret_obj.name_y] + obj.data_table[obj.name_y]
        )
        return ret_obj
    ...
    def obtainValue(self, value):
        ...
        """
        index_value = (
            (self.data_table[self.name_x] - value).abs().sort_values().index[0]
        )
    ...
    def obtainMax(self, low=None, high=None, Print=False):
        ...
        Paramaters
        ----------
        low: float minimum value of x range

        high: float maximum value of x range

        Print: bool print the results out
        ...
        """
        low_index, high_index = self._minMaxIndex(low, high)
        maxi_index = self.data_table[self.name_y][
            low_index:high_index
        ].idxmax()
        if Print:
            print(
                "the maximum is at: "
                + str(self.data_table[self.name_x][maxi_index])
                + "; And the value is: "
                + str(self.data_table[self.name_y][maxi_index])
            )
        return (
            self.data_table[self.name_x][maxi_index],
            self.data_table[self.name_y][maxi_index],
        )
    ...
    def obtainMin(self, low=None, high=None, Print=False):
        ...
        returns
        -------
        float, float x-position; y-minimum
        """
        low_index, high_index = self._minMaxIndex(low, high)
        ...
        if Print:
            print(
                "the minimum is at: "
                + str(self.data_table[self.name_x][min_index])
                + "; And the value is: "
                + str(self.data_table[self.name_y][min_index])
            )
        return (
            self.data_table[self.name_x][min_index],
            self.data_table[self.name_y][min_index],
        )

    def forcePositive(self, itself=True):
        """add minimum value of the y vector to itself and thus force it to be
        positive"""
        self.positive = True
        ...
        if mini < 0:
            if itself:
                self.data_table[self.name_y] = (
                    self.data_table[self.name_y] - mini
                )
            else:
                spec = BasicSpectrum(
                    self.data_table[self.name_x],
    ...
    def normalize(self, wavelength=None, low=None, high=None, itself=True):
        """normalize the spectra so that the wavelenght value is 1. If
        wavelenght is None the maximum of the range between low and high values
        will be use to normalize the spectrum.

        Paramaters
        ----------
        wavelength: float wavelenth value where the spectra shoudl be 1 if None
            the maximum will be chose

        low: float minimum value of x range

        high: float maximum value of x range

        itself: bool if true normalize the spectrum else returns a new spectrum
            object
        """
    ...
    def plotSpectrum(
        ...
        **kwargs,
        ...
        Paramaters
        ----------
        axes: str or matplotlib axes object -  "current" plots in the last open
              axes -  "new" create a new axes -  pass a matplotlib axes where
              the spectra will be plot

        x_label: str the x label of the plot

        y_label: str the y label of the plot

        label: str the legend to be display (pass "_" form no label)

        fill: bool fill area under the spectrum curve

        two_Xaxes: bool in case two x is tranformed (i.e. wavelenght and
            wavenumber) and if True, the old and new axis will be plot

        kwargs: dict matplot lib kwargs for plt.plot()

        extra_kwargs: dict this parameters can be pases as kwargs eve if they
            are not from plt.plot() fsize: 14 small_tick: True which:both
            direction:in
        ...
        ax1.axes.tick_params(
            which="both",
            direction="in",
            top=True,
            right=True,
            labelsize=plot_properties["fsize"],
        )
        ...
        plt.tick_params(
            axis="both", which="major", labelsize=plot_properties["fsize"]
        )
        plt.xlim(
            self.data_table[self.name_x][self.data_table.index[0]],
            self.data_table[self.name_x][self.data_table.index[-1]],
        )
    ...
    def baselineCorrection(self, low=None, high=None, itself=True):
        """subrtact the average value of a range to the spectra acording to
        values of x
        ...
        itself: bool if true subtract the spectrum else returns a new spectrum
            object
