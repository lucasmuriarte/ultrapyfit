...
class ModelCreator:
    """
    Class containing the simple exponential and sum of exponential functions (including modified
    ...
    def __init__(self, exp_no, time, tau_inf=1E+12):
        ...
        tau_inf: float or int (default 1E12)
            extra time constant to evaluate Gauss modified exponential functions that have not
            completely decay at long decay times (off-set).
        """
        self.exp_no = exp_no
        self.x = time
    ...
    def expN(time, y0, t0, values):
        ...
        1darray of size equal to time-vector 
        """
        return y0+sum([pre_exp*ModelCreator.exp1(time-t0, tau) for pre_exp, tau in values])
    ...
    def expNGauss(time, y0, t0, fwhm, values):
        ...
        """
        return y0+sum([pre_exp*ModelCreator.expGauss(time-t0, tau, fwhm/2.35482) for pre_exp, tau in values])
    ...
    def expNGaussDataset(self, params, i):
        ...
        values = [[params['pre_exp%i_' % (ii+1)+str(i+1)].value, params['tau%i_' % (ii+1)+str(i+1)].value] for ii in
                  range(self.exp_no)]
    ...
    def expNDataset(self, params, i):
        ...
        values = [[params['pre_exp%i_' % (ii+1)+str(i+1)].value, params['tau%i_' % (ii+1)+str(i+1)].value]
