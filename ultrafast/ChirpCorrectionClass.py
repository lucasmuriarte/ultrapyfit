...
class ChripCorrection:
    ...
    def fprime(self, x, b1, b2, b3, c1, c2, c3):
        return (
            ...
            + 1.0 * b1 * x / (-(c1 ** 2) + x ** 2)
            - 1.0 * b2 * x ** 3 / (-(c2 ** 2) + x ** 2) ** 2
            + 1.0 * b2 * x / (-(c2 ** 2) + x ** 2)
            - 1.0 * b3 * x ** 3 / (-(c3 ** 2) + x ** 2) ** 2
            + 1.0 * b3 * x / (-(c3 ** 2) + x ** 2)
        )
    ...
    def dispersion(self, landa, element, excitation):
        ...
        n_g = np.array(
            [
                self.indexDifraction(i, b1, b2, b3, c1, c2, c3)
                - i * self.fprime(i, b1, b2, b3, c1, c2, c3)
                for i in landa
            ]
        )
        n_excitation = self.indexDifraction(
            excitation, b1, b2, b3, c1, c2, c3
        ) - excitation * self.fprime(excitation, b1, b2, b3, c1, c2, c3)
        # 0.299792458 is the speed of light transform to correct units
        GVD = 1 / 0.299792458 * (n_excitation - n_g)
        return GVD

    def GVD(self, CaF2=0, SiO2=0, BK7=0, offset=0):
        if self.dispersion_BK7 is 0:
            self.dispersion_BK7 = self.dispersion(
                self.wavelength, self._BK7_param, self.excitation
            )
        if self.dispersion_Caf2 is 0:
            self.dispersion_Caf2 = self.dispersion(
                self.wavelength, self._CaF2_param, self.excitation
            )
        if self.dispersion_SiO2 is 0:
            self.dispersion_SiO2 = self.dispersion(
                self.wavelength, self._SiO2_param, self.excitation
            )
        print(CaF2, SiO2, BK7, offset)
        self.gvd = (
            self.dispersion_BK7 * BK7
            + self.dispersion_SiO2 * SiO2
            + self.dispersion_Caf2 * CaF2
            + offset
        )
        self.CaF2, self.BK7, self.SiO2, self.GVD_offset = (
            CaF2,
            BK7,
            SiO2,
            offset,
    ...
    def GVDFromPolynom(self, qt=None):
        ...
        self.figGVD.canvas.mpl_connect(
            "button_press_event", self.cursor_pol.onClick
        )
        self.figGVD.canvas.mpl_connect(
            "motion_notify_event", self.cursor_pol.mouseMove
        )
        self.figGVD.canvas.mpl_connect(
            "axes_enter_event", self.cursor_pol.onEnterAxes
        )
        self.figGVD.canvas.mpl_connect(
            "axes_leave_event", self.cursor_pol.onLeaveAxes
        )
        resetax2 = plt.axes([0.70, 0.025, 0.1, 0.04])
        resetax = plt.axes([0.85, 0.025, 0.1, 0.04])
        self.button = Button(
            resetax, "Calculate", color="tab:red", hovercolor="0.975"
        )
        self.button2 = Button(
            resetax2, "fit", color="tab:red", hovercolor="0.975"
        )
    ...
    def fitPolGVD(self, event):
        ...
        wavelength = self.cursor_pol.x
        out = lmfit.minimize(
            optimize, params, args=(np.array(x), point_pol_GVD, 3)
        )
    ...
    #        print('top') point_pol_GVD=experiment.cursor_pol.datay def
    #        optimize(params,x,y,order): print(order) return
    #        (y-polynomi(params,x,order)) params=lmfit.Parameters()
    #        params.add('c0',value=-5.7,max=0)
    #        params.add('c1',value=0.028,min=0)
    #        params.add('c2',value=-4.22E-5,max=0)
    #        params.add('c3',value=2.153E-8,min=0)
    #
    #        poly=polynomi(params,wavelength,3) experiment.l.set_ydata(poly)
    #        experiment.figGVD.canvas.draw()
    #
    #        x=experiment.cursor_pol.datax wavelength=experiment.cursor_pol.x
    #        out = lmfit.minimize(optimize, params,
    #        method='nelder',args=(np.array(x),point_pol_GVD,3)) poly =
    #        polynomi(out.params,wavelength,3) experiment.l.set_ydata(poly)
    #        experiment.figGVD.canvas.draw() para=out.params
    #        self.polynom_gvd=[out.params[key] for key in out.params]
    #        experiment.polynomGVD=True experiment.l.set_ydata(experiment.gvd)
    #        experiment.figGVD.canvas.draw() print('top')
    ...
    def updateGVD(self, val):
        ...
        # update curve
        self.l.set_ydata(
            bk * self.dispersion_BK7
            + caf2 * self.dispersion_Caf2
            + sio2 * self.dispersion_SiO2
            + ofset
        )
    ...
    def verifiedGVD(self):
        assert (
            self.GVD_correction == "in process"
        ), "Please first try to correct the GVD"
        axcolor = "lightgoldenrodyellow"
        ...
        ax0.pcolormesh(
            self.wavelength,
            self.x[: self.index2ps],
            pd.DataFrame(self.corrected_data).iloc[: self.index2ps].values,
            cmap="RdYlBu",
        )
        # ax1 = self.fig.add_subplot(1,2,2) self.corrected_data
        ...
        resetax = plt.axes([0.85, 0.025, 0.1, 0.04])
        self.button = Button(
            resetax, "Apply", color="tab:red", hovercolor="0.975"
        )
