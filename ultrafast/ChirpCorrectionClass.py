...
class ChripCorrection:
    ...
    def indexDifraction(self, x, b1, b2, b3, c1, c2, c3):
        n = (1 + (b1 * x**2 / (x**2 - c1**2)) + (b2 * x**2 /
                                                 (x**2 - c2**2)) + (b3 * x**2 / (x**2 - c3**2)))**0.5
        return n
    ...
    def dispersion(self, landa, element, excitation):
        ...
        n_g = np.array([self.indexDifraction(i,
                                             b1,
                                             b2,
                                             b3,
                                             c1,
        ...
        n_excitation = self.indexDifraction(
            excitation, b1, b2, b3, c1, c2, c3) - excitation * self.fprime(
    ...
    def GVD(self, CaF2=0, SiO2=0, BK7=0, offset=0):
        ...
        self.gvd = self.dispersion_BK7 * BK7 + self.dispersion_SiO2 * \
            SiO2 + self.dispersion_Caf2 * CaF2 + offset
                ...
                if valor < self.x[0]:
                    ...
                    if value == valor:
                        ...
                        if idex == len(new_time) - 1:
                            ...
                            corrected_data[ii, i] = w * self.data[idex,
                                                                  i] + (1 - w) * self.data[idex + 1, i]
                    else:
                        valores[ii, i] = +1
                        inf = self.x[idex - 1]
                        sub = self.x[idex]
                        ...
                        corrected_data[ii,
                                       i] = w * self.data[idex - 1,
                                                          i] + (1 - w) * self.data[idex,
                                                                                   i]
...
#        pcolormesh(self.wavelength,self.x[:46],pd.DataFrame(corrected_data).iloc[:46].values,cmap='RdYlBu')
    ...
    def GVDFromPolynom(self, qt=None):
        ...
        self.ax.pcolormesh(self.wavelength, self.x[:self.index2ps], pd.DataFrame(
            self.data).iloc[:self.index2ps].values, cmap='RdYlBu')
        plt.axis([self.wavelength[0], self.wavelength[-1],
                  self.x[0], self.x[self.index2ps - 1]])
        plt.subplots_adjust(bottom=0.15)
        self.cursor_pol = SnaptoCursor(self.ax,
                                       self.wavelength,
...
#        plt.close(self.fig)
        ...
        wavelength = self.cursor_pol.x
        out = lmfit.minimize(
            optimize, params, args=(
                np.array(x), point_pol_GVD, 3))
...
#
    ...
    def GVDFromGrapth(self, qt=None):
        ...
        if self.time_unit == 'ns':
            value = value / 1000
        self.index2ps = np.argmin([abs(i - value) for i in self.x])
        pcolormesh(self.wavelength, self.x[:self.index2ps], pd.DataFrame(
            self.data).iloc[:self.index2ps].values, cmap='RdYlBu')
    ...
    def updateGVD(self, val):
        ...
        self.l.set_ydata(
            ...
            self.dispersion_BK7 +
            caf2 *
            self.dispersion_Caf2 +
            sio2 *
            self.dispersion_SiO2 +
            ofset)
    ...
    def verifiedGVD(self):
        ...
        ax0.pcolormesh(self.wavelength, self.x[:self.index2ps], pd.DataFrame(
            self.corrected_data).iloc[:self.index2ps].values, cmap='RdYlBu')
        #ax1 = self.fig.add_subplot(1,2,2)
#        self.corrected_data
