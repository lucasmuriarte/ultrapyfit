    ...
    def objectiveTagetFit(params,shared_t0=True)  :
        ...
        if self.deconv:
            if shared_t0:
                ...
                expvects = [coeffs[i]*self.expGauss(self.x-t0,-1/eigs[i],fwhm/2.35482) for i in range(len(eigs))] 
                concentrations = [sum([eigenmatrix[i,j]*expvects[j] for j in range(len(eigs))]) for i in range(len(eigs))] 
                resid=self.generateResidues(self.expNGaussDatasetFast,params,concentrations) 
            else: 
                resid=self.generateResidues(expNGaussDatasetTM,params, (coeffs,eigs,eigenmatrix))
        else:
            t0 = params['t0_1'].value
            index=np.argmin([abs(i-t0) for i in self.x])
            values=[params['tau%i_1' %(ii+1)].value for ii in range(self.exp_no)] 
            expvects=[self.exp1(self.x-t0,tau) for tau in values]
            resid=self.generateResidues(self.expNGaussDatasetTM, params, (coeffs,eigs,eigenmatrix))[index:,:]   
